<script>
  ...
  function clear() {
    ...
    status = 'all'
    search = ''
    router.get('/reports/payables', {}, { preserveScroll: true, replace: true })
  }
  ...
  <div class="space-y-5">
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">Compras y documentos soporte pendientes de pago</p>
      </div>
      <a href="/cash/receipts?type=payment"
        ...
        <i class="mdi mdi-cash-refund text-base"></i>
        Nuevo egreso
      </a>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-4">
      <div class="grid grid-cols-1 gap-3 md:grid-cols-[160px_1fr_180px_1fr_auto_auto] md:items-end">
        ...
        <div>
          ...
          <input type="search" bind:value={search} placeholder="Documento, proveedor o NIT"
            class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
    ...
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
      <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-xs text-slate-500 mb-1">Saldo por pagar</p>
      ...
      <section class="xl:col-span-2 bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        {#if documents.data.length === 0}
          <div class="py-16 text-center text-slate-400 text-sm">
            <i class="mdi mdi-file-clock-outline text-4xl block mb-2"></i>
            ...
            <table class="w-full text-sm">
              <thead class="bg-slate-50">
                <tr class="text-xs font-semibold text-slate-500 uppercase tracking-wide text-left">
                  ...
                  <th class="px-4 py-2.5 text-right">Pagado</th>
                  ...
                  <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-4 py-3 text-right">
                      <button onclick={() => pay(row)}
                        ...
                        <i class="mdi mdi-cash-minus text-sm"></i>
                        Pagar
                      </button>
                    </td>
                  </tr>
      ...
      <aside class="space-y-5">
        <section class="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div class="px-5 py-3 border-b border-slate-100">
            <h2 class="text-sm font-semibold text-slate-700">Edad de proveedores</h2>
          </div>
        ...
        <section class="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div class="px-5 py-3 border-b border-slate-100">
            <h2 class="text-sm font-semibold text-slate-700">Proveedores con mayor saldo</h2>
          </div>
              ...
              {#each providers as provider}
                <li class="px-5 py-3">
                  <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <p class="truncate text-sm font-semibold text-slate-700">{provider.name}</p>
                      <p class="text-xs text-slate-400">{provider.documents} docs · mora máx. {provider.max_days_overdue} días</p>
                    </div>
                    <p class="text-sm font-bold text-slate-800 tabular-nums">{fmt(provider.balance)}</p>
                  </div>
                  {#if provider.overdue > 0}
                    <p class="mt-1 text-xs text-rose-600">Vencido: {fmt(provider.overdue)}</p>
                  {/if}
                </li>
              {/each}
            </ul>
