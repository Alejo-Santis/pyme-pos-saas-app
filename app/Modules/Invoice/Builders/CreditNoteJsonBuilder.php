...
/**
 * Constructor del JSON UBL 2.1 para Notas Crédito.
 */
class CreditNoteJsonBuilder
{
    public static function fromDocument(Document $document, bool $sendmail = true): array
    {
        ...
        $json = [
            ...
            // Factura de referencia (obligatoria en NC)
            'billing_reference' => [
            ...
            // Razón de la nota crédito (discrepancy)
            'discrepancy_response' => [
                'correction_concept_id' => $document->credit_note_discrepancy_id ?? 1,
                'description'           => $document->credit_note_reason ?? 'Devolución parcial',
            ],
            ...
            'credit_note_lines'     => (new InvoiceLines($document->invoice_lines ?? [], $document->type_document_operation_id))->toArray(),
        ];
